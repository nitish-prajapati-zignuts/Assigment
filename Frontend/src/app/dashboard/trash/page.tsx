...
export default function TrashPage() {
  ...
  const [meetingToPermanentDelete, setMeetingToPermanentDelete] = useState<string | null>(null);

  // TanStack Query for Trashed Meetings
  ...
  const permanentDeleteMutation = useMutation({
    ...
    onError: () => {
      toast.error("Failed to delete meeting permanently");
    },
  });
    ...
    <div className="space-y-8 mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            View meeting notes stored in the trash. Items can be restored or permanently removed.
          </p>
        </div>
      </div>
      ...
      <Card className="rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-md shadow-2xs">
        <CardContent className="p-4 sm:p-5">
          <div className="relative w-full max-w-md">
            ...
            <Input
              placeholder="Search trashed title, participant, transcript..."
              value={searchQuery}
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center bg-white/70 dark:bg-zinc-900/70 rounded-3xl border border-zinc-200/80 dark:border-zinc-800/80 flex flex-col items-center justify-center space-y-4">
          ...
          <div>
            <p className="text-sm font-bold text-zinc-800 dark:text-zinc-200">Trash Bin is empty.</p>
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">
              {searchQuery ? "Try refining your search keyword." : "Deleted meetings will appear here temporarily."}
            </p>
          </div>
        </div>
      ) : (
                ...
                <Card className="rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-300 shadow-xs flex flex-col justify-between h-[210px]">
                  ...
                  <CardContent className="px-5 pb-5 pt-0 flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800/50 pt-3.5 mt-4">
                      {meetingToPermanentDelete === meeting.id ? (
                        <div className="flex items-center gap-2 w-full justify-between">
                          <span className="text-[10px] text-red-500 font-bold flex items-center gap-1">
                            <AlertTriangle className="h-3 w-3 shrink-0" /> Are you sure?
                          </span>
                          <div className="flex items-center gap-1.5">
                            <Button
                              size="xs"
                              variant="destructive"
                              onClick={() => permanentDeleteMutation.mutate(meeting.id)}
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              className="h-8 w-8 p-0 text-zinc-650 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 rounded-xl"
                            ...
                            <Button
                              size="xs"
                              variant="ghost"
                              onClick={() => setMeetingToPermanentDelete(meeting.id)}
